from fastapi import APIRouter, Depends, HTTPException, status
from src.models.schemas.users.users_request import UsersRequest
from services.authorization import AuthorizationService
from src.models.schemas.utils.jwt_token import JwtToken
from fastapi.security import OAuth2PasswordRequestForm


router = APIRouter(
    prefix='/authorization',
    tags=['Authorization']
)


@router.post('/register', status_code=status.HTTP_201_CREATED, name='Регистрация')
def register(user_schema: UsersRequest, authorize_service: AuthorizationService = Depends()):
    """
    Регистрация нового пользователя
    """

    return authorize_service.register(user_schema)


@router.post('/authorize', response_model=JwtToken, name='Авторизация')
def authorization(auth_schema: OAuth2PasswordRequestForm = Depends(), authorization_service: AuthorizationService = Depends()):
    """
    Авторизация пользователя
    """
    result = authorization_service.authorize(auth_schema.username, auth_schema.password)
    if not result:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Не авторизован')
    
    return result
